...
public sealed class CreateBodyPartCommand : LocalizedCommands
{
    ...
    public override string Command => "createbodyproviderslot";

    public override void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        var providerType = BodyProviderType.None;
        ...
        if (args.Length == 3 && !Enum.TryParse(args[2], out providerType))
        {
        ...
        if (!bodySystem.TryCreateProviderSlot(uid, args[1], providerType))
        {
    ...
    public override CompletionResult GetCompletion(IConsoleShell shell, string[] args)
    {
        ...
        if (args.Length == 3)
            return CompletionResult.FromHintOptions(WhiteCompletionHelper.Emuns(typeof(BodyProviderType)), Loc.GetString("shell-argument-body-provider-type-optional-hint"));
