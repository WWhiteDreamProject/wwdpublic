...
public sealed class CreateBoneSlotCommand : LocalizedCommands
{
    [Dependency] private readonly IEntityManager _entManager = default!;

    public override string Command => "createboneslot";
    ...
    public override void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        if (args.Length == 3 && !Enum.TryParse(args[2], out boneType))
        {
            shell.WriteError(Loc.GetString("shell-argument-bone-type-invalid", ("index", args[2])));
            return;
        }
        ...
        if (!bodySystem.TryCreateBoneSlot(uid, args[1], boneType))
        {
    ...
    public override CompletionResult GetCompletion(IConsoleShell shell, string[] args)
    {
        ...
        if (args.Length == 3)
            return CompletionResult.FromHintOptions(WhiteCompletionHelper.Emuns(typeof(BoneType)), Loc.GetString("shell-argument-bone-type-optional-hint"));
