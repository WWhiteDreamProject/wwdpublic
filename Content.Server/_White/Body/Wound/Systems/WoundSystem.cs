...
namespace Content.Server._White.Body.Wound.Systems;
...
public sealed class WoundSystem : SharedWoundSystem
{
    ...
    private void OnHealingDoAfter(Entity<WoundableComponent> woundable, ref HealingDoAfterEvent args)
    {
        ...
        if (TryComp<BloodstreamComponent>(woundable, out var bloodstream))
        {
            ...
            {
                _bloodstream.TryModifyBleedAmount(woundable.Owner, healing.BloodlossModifier, args.BodyPartType);

                if (bloodstream.Bleeding == 0)
                {
            ...
            if (healing.ModifyBloodLevel != 0)
                _bloodstream.TryModifyBloodLevel((woundable, bloodstream), healing.ModifyBloodLevel);
        }
