...
public sealed partial class ClampedHsvColoration : ISkinColorationStrategy
{
    ...
    public bool VerifySkinColor(Color color)
    {
        ...
        if (Hue is var (minHue, maxHue) && !ISkinColorationStrategy.IsHueInRange(hsv.X, minHue, maxHue))
            return false;

        if (Saturation is var (minSat, maxSat) && (hsv.Y < minSat - ISkinColorationStrategy.Epsilon || hsv.Y > maxSat + ISkinColorationStrategy.Epsilon))
            return false;

        return Value is not var (minVal, maxVal) || !(hsv.Z < minVal - ISkinColorationStrategy.Epsilon) && !(hsv.Z > maxVal + ISkinColorationStrategy.Epsilon);
    }
    ...
    public Color ClosestSkinColor(Color color)
    {
        ...
        if (Hue is var (minHue, maxHue))
            hsv.X = ISkinColorationStrategy.ClampHue(hsv.X, minHue, maxHue);
