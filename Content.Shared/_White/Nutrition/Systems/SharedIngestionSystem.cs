...
public abstract partial class SharedIngestionSystem : EntitySystem
{
    ...
    protected EntityQuery<IngestibleComponent> IngestibleQuery;
    ...
    public override void Initialize()
    {
        ...
        IngestibleQuery = GetEntityQuery<IngestibleComponent>();
    }
    ...
    /// <param name="user">The entity performing the action (e.g., attempting to feed another).</param>
    ...
    {
        if (!IngestibleQuery.Resolve(ent, ref ent.Comp, false))
            return false;

        return CanConsume((ent, ent.Comp), user, target, out _);
    }
...
public record struct AttemptIngestedEvent(bool Cancelled = false) : IInventoryRelayEvent
{
    ...
    /// </summary>
    public SlotFlags TargetSlots { get; } = SharedIngestionSystem.DefaultFlags;
...
public record struct GetUtensilsEvent(UtensilType Type)
{
    ...
    /// </summary>
    public List<Entity<UtensilComponent>> Utensils = default!;
