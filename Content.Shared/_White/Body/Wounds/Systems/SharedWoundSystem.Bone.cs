...
public abstract partial class SharedWoundSystem
{
    private void InitializeBone() =>
        SubscribeLocalEvent<WoundableBoneComponent, BoneRelayedEvent<RejuvenateEvent>>(OnBoneRejuvenate);

    #region Event Handling

    private void OnBoneRejuvenate(Entity<WoundableBoneComponent> woundableBone, ref BoneRelayedEvent<RejuvenateEvent> args)
    {
    ...
    private void ApplyBoneDamage(Entity<BoneComponent?, WoundableBoneComponent?> bone, DamageSpecifier damage)
    {
        ...
        RaiseLocalEvent(bone, new BoneHealthChangedEvent((bone, bone.Comp2), totalDamageDelta));
