...
public abstract class SharedImplanterSystem : EntitySystem
{
    ...
    {
        if (!CanImplant(user, target, implanter, component, out var implant, out var implantComp, out _)) // WD EDIT
            return;
        ...
        if (_container.TryGetContainer(target, ImplanterComponent.ImplantSlotId, out var implantContainer))
        {
            ...
            if (component.AllowDeimplantAll)
            {
                foreach (var implant in implantContainer.ContainedEntities)
                {
                    ...
                    DrawImplantIntoImplanter(implanter, user, target, implant, implantContainer, implanterContainer, implantComp); // WD EDIT
                    permanentFound = implantComp.Permanent;
            ...
            else
            {
                ...
                if (implant != null && implantCompQuery.TryGetComponent(implant, out var implantComp))
                {
                    ...
                    {
                        DrawImplantIntoImplanter(implanter, user, target, implant.Value, implantContainer, implanterContainer, implantComp); // WD EDIT
                        permanentFound = implantComp.Permanent;
                    }
    ...
    private void DrawImplantIntoImplanter(EntityUid implanter, EntityUid user, EntityUid target, EntityUid implant, BaseContainer implantContainer, ContainerSlot implanterContainer, SubdermalImplantComponent implantComp) // WD EDIT
    {
