...
forensic-pad-no-fingerprints = { CAPITALIZE($target) } не оставляет отпечатков!
forensic-pad-start-scan-target = { CAPITALIZE($user) } пытается снять отпечатки ваших пальцев.
forensic-pad-start-scan-user = Вы начинаете снимать отпечатки пальцев с { CAPITALIZE($target) }.
