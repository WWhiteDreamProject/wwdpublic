ent-BaseWeaponShotgun = Дробовик
    .desc = Дерзай — налетай, целься и стреляй.
    .suffix = Дробовик, debug


ent-WeaponShotgunBulldog = «Бульдог»
    .desc = Автоматический дробовик с магазинным питанием, идеальный для ближнего боя. Использует патроны калибра .50 ружейный.
    .suffix = Дробовик

ent-WeaponShotgunEnforcer = «Силовик»
    .desc = Боевой дробовик премиум-класса, созданный на основе конструкции Каммерера, с полуавтоматической системой заряжания. Использует патроны калибра .50 ружейный.
    .suffix = Дробовик

ent-WeaponShotgunEnforcerRubber = { ent-WeaponShotgunEnforcer }
    .desc = { ent-WeaponShotgunEnforcer.desc }
    .suffix = Дробовик, Травматический

ent-WeaponShotgunDoubleBarreledRubber = двуствольное ружьё
    .desc = Бессмертная классика. Использует патроны калибра .50 ружейный.
    .suffix = Дробовик, Травматический

ent-WeaponShotgunDoubleBarreled = двуствольное ружьё
    .desc = Бессмертная классика. Использует патроны калибра .50 ружейный.
    .suffix = Дробовик

ent-WeaponShotgunKammerer = «Каммерер»
    .desc = Вот что выходит, когда классический дизайн Ремингтона соединяется с современными материалами. Использует патроны калибра .50 ружейный.
    .suffix = Дробовик

ent-WeaponShotgunSawn = обрез
    .desc = Против всякой шелупони. Использует патроны калибра .50 ружейный.
    .suffix = Дробовик

ent-WeaponShotgunSawnEmpty = обрез
    .desc = Против всякой шелупони. Использует патроны калибра .50 ружейный.
    .suffix = Дробовик, Пустой

ent-WeaponShotgunSawnLumen = обрез
    .desc = Против всякой шелупони. Использует патроны калибра .50 ружейный.
    .suffix = Дробовик, Люмен .50

ent-WeaponShotgunHandmade = самодельный пистолет
    .desc = Выглядит ненадежно. Использует патроны калибра .50 ружейный.
    .suffix = Дробовик, Пустой

ent-WeaponShotgunBlunderbuss = мушкетон
    .desc = Смертельно опасен на близком расстоянии.
    .suffix = Дробовик, Пират

ent-WeaponShotgunImprovised = самодельный дробовик
    .suffix = Дробовик, Пустой
    .desc = Дерьмовенькое ружьё кустарного производства, использующее патроны калибра .50 ружейный. Патронник рассчитан только на один патрон.

ent-WeaponShotgunImprovisedLoaded = самодельный дробовик
    .suffix = Дробовик, Заряжен
    .desc = { ent-WeaponShotgunImprovised.desc }

ent-WeaponShotgunBulldogEmpty = «Бульдог»
    .desc = Автоматический дробовик с магазинным питанием, идеальный для ближнего боя. Использует патроны калибра .50 ружейный.
    .suffix = Дробовик, Пустой

ent-WeaponShotgunBulldogHoS = трофейный «Бульдог»
    .desc = Автоматический дробовик с магазинным питанием, идеальный для ближнего боя. Использует патроны калибра .50 ружейный.
    .suffix = Дробовик, Пустой

ent-WeaponShotgunKammererNonLethal = «Каммерер»
    .desc = Вот что выходит, когда классический дизайн Ремингтона соединяется с современными материалами. Использует патроны калибра .50 ружейный.
    .suffix = Дробовик, Травматический

ent-WeaponShotgunHandmadeEmpty = самодельный пистолет
    .desc = Выглядит ненадежно. Использует патроны калибра .50 ружейный.
    .suffix = Дробовик, Пустой

ent-WeaponShotgunAD11Cerberus = «AD-11 „Церберус“»
    .desc = Автоматическое ружье с барабанным магазином. Переклассифицированы в «оружие массового поражения» после десятков «трудоубийств», после чего перестали поставляться в подразделения безопасности на станциях Корпорации. Неизвестным образом большие излишки этого оружия стали широко использоваться Синдикатом.
    .suffix = Дробовик

ent-WeaponShotgunAdjutant = «Адъютант»
    .desc = Презентуемый как средство подавления беспорядков, этот дробовик оснащён специальной системой заряжания, делающей его идеальным для ближнего боя.
    .suffix = Дробовик
