...
namespace Content.Client._White.BloodCult.Runes.UI;
...
public sealed class RuneDrawerBUI : BoundUserInterface
{
    ...
    protected override void UpdateState(BoundUserInterfaceState state)
    {
        if (state is RuneDrawerMenuState runeDrawerState)
            FillMenu(runeDrawerState.AvailableRunes);
    }

    private void FillMenu(List<ProtoId<BloodCultRunePrototype>>? runes = null)
    {
        if (runes is null)
            return;
