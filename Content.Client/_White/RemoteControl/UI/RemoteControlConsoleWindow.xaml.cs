...
public sealed partial class RemoteControlConsoleWindow : FancyWindow, IComputerWindow<NavInterfaceState>
{
    ...
    public RemoteControlConsoleWindow()
    {
        ...
        HUDHolder.OnMouseWheel += (delta) => CameraScale = Math.Clamp(CameraScale + delta / 5f, MinCameraScale, MaxCameraScale);
    ...
    public void UpdateState(NavInterfaceState scc, IEye? eye, float minScale, float maxScale, RemoteControlVisualMode mode, RemoteControlConsoleError error)
    {
        RadarScreen.UpdateState(scc);
        _turretEye = eye;
        MinCameraScale = minScale;
        MaxCameraScale = maxScale;
...
// stupid invisible control for intercepting all user input in a single place
// instead of having to handle user input on 2 different controls
