...
public sealed class ActionUIController : UIController, IOnStateChanged<GameplayState>, IOnSystemChanged<ActionsSystem>
{
    ...
    private readonly List<EntityUid?> _actions = new(); // WD EDIT
    ...
    {
        if (_actionsSystem == null ||
            !_actions.TryGetValue(index, out var actionId) || // WD EDIT
            !_actionsSystem.TryGetActionData(actionId, out var baseAction))
            return;

        if (baseAction is BaseTargetActionComponent action)
            ToggleTargeting(actionId.Value, action); // WD EDIT
        else
            _actionsSystem?.TriggerAction(actionId.Value, baseAction); // WD EDIT
    }
    ...
    private void OnActionRemoved(EntityUid actionId)
    {
        ...
        _actions.RemoveAll(x => x == actionId); // WD EDIT
    }
    ...
    private void OnActionsUpdated()
    {
        ...
        if (_actionsSystem != null)
            _container?.SetActionData(_actionsSystem, _actions.ToArray()); // WD EDIT
    }
    ...
    private void SetAction(ActionButton button, EntityUid? actionId, bool updateSlots = true) // WD EDIT
    {
        if (_actionsSystem == null)
            return;
    ...
    private void DragAction()
    {
        ...
        if (currentlyHovered is ActionButton button)
        {
            ...
            SetAction(button, action, false); // WD EDIT
        }

        if (dragged.Parent is ActionButtonContainer)
            SetAction(dragged, swapAction, false); // WD EDIT
